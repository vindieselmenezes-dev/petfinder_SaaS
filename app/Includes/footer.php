...
<script src="<?= Url::asset('js/geolocalizacao.js') ?>"></script>
<script src="<?= Url::asset('js/click-sounds.js') ?>"></script>
<?php
require_once __DIR__ . '/../Helpers/Csrf.php';
$pushVapidKey = getenv('PUSH_VAPID_PUBLIC_KEY') ?: '';
if ($pushVapidKey && Auth::check()):
    ?>
    <script>
...
<script>
    if (window.PetfinderLocalizacao) {
        window.PetfinderLocalizacao.solicitarSeNecessario('<?= Url::ajax('salvar_localizacao.php') ?>');
    }
</script>
