...
class SolicitacaoAdocao
{
    ...
    public function buscarPorId(int $id): ?array
    {
        $stmt = $this->pdo->prepare("
            SELECT
                sa.id, sa.pet_id, sa.usuario_solicitante_id, sa.conversa_id,
                sa.mensagem, sa.status, sa.criado_em, sa.atualizado_em,
                p.nome AS pet_nome, p.usuario_id AS pet_dono_id,
    ...
    public function listarEnviadas(int $usuarioId): array
    {
        $stmt = $this->pdo->prepare("
            SELECT
                sa.id, sa.pet_id, sa.usuario_solicitante_id, sa.conversa_id,
                sa.mensagem, sa.status, sa.criado_em, sa.atualizado_em,
                p.nome AS pet_nome, p.foto AS pet_foto, p.status AS pet_status
            FROM solicitacoes_adocao sa
            INNER JOIN pets p ON p.id = sa.pet_id
