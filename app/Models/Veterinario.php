...
class Veterinario
{
    ...
    public function buscarPorUsuario(int $usuarioId): ?array
    {
        $stmt = $this->pdo->prepare("
            SELECT id, usuario_id, crmv, biografia, experiencia, valor_consulta,
                   atendimento_domicilio, atendimento_online, foto, avaliacao,
                   total_avaliacoes, ativo, criado_em, atualizado_em
            FROM veterinarios WHERE usuario_id = ?
        ");
