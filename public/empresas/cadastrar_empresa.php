...
/**
 * ==========================================================
 * ADICIONAR EMPRESA (fluxo logado)
 * ==========================================================
 * Usada por um usuário JÁ logado para cadastrar mais uma empresa na
 * própria conta (ex: dono de duas clínicas). Exige login.
 *
 * Não confundir com cadastro_empresa.php, que é o cadastro público
 * completo (cria usuário + empresa) para quem ainda não tem conta.
 * ==========================================================
 */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!Csrf::validar($_POST["csrf_token"] ?? null)) {
        ...
        $dados["plano_id"] = $planoValido ? (int) $planoValido["id"] : null;
        ...
        */
        ...
        if ($novoId !== false) {
            ...
            */
            ...
            // Plano pago escolhido no cadastro: aplica o trial via o mesmo
            // fluxo simulado de assinatura já usado no painel (calcula
            // plano_expira_em e grava na auditoria).
            if ($planoValido && (float) $planoValido["preco_mensal"] > 0) {
                header("Location: simular_assinatura.php?empresa_id=" . $novoId . "&plano_id=" . (int) $planoValido["id"]);
                exit;
            }
