...
$stmtAcesso = $pdo->prepare("
    SELECT e.id AS empresa_id, e.nome_fantasia, e.ativo, e.status_pagamento, e.plano_id, p.nome AS plano_nome, p.destaque AS plano_destaque, ee.papel
...
if ($empresaData) {
    ...
    $stmtEmpCheck = $pdo->prepare("
        SELECT e.id AS empresa_id, e.nome_fantasia, e.ativo, e.status_pagamento, e.plano_id, p.nome AS plano_nome, p.destaque AS plano_destaque
        FROM empresas e
        LEFT JOIN planos p ON p.id = e.plano_id
        WHERE e.id = ?
    ");
    ...
    if ($dbEmp) {
        ...
        $empresaData = [
            ...
            'plano_id' => $dbEmp['plano_id'],
            'plano_nome' => $dbEmp['plano_nome'],
            'plano_destaque' => $dbEmp['plano_destaque'],
...
// Empresa fica em modo somente-leitura se estiver inativa, com o
// faturamento atrasado/suspenso (empresas.status_pagamento), OU se o
// período de teste do plano já venceu sem assinar um plano pago.
$statusPagamento = $empresaData['status_pagamento'] ?? 'Ativo';
$trialVencido = (new EmpresaController())->trialVencido($empresaId);
$readOnly = empty($empresaData['ativo']) || $statusPagamento !== 'Ativo' || $trialVencido;
...
<main class="container"
    ...
    <div style="margin-bottom: 30px;">
        ...
        <?php endif; ?>
        <a href="<?= Url::pagina('meus_produtos.php') ?>?empresa_id=<?php echo $empresaId; ?>" class="btn"
        ...
        <a href="<?= Url::pagina('consultas_empresa.php') ?>?empresa_id=<?php echo $empresaId; ?>" class="btn"
            style="background: #015C1E; color: white; padding: 10px 20px; text-decoration: none; border-radius: 4px; display: inline-block; margin-left: 10px;">🩺
            Consultas Agendadas</a>
    </div>
        ...
        foreach ($historicos as $reg):
            ...
            <div class="record-card"
                ...
                <div class="record-body">
                    ...
                    <?php endif; ?>
                    <?php if (!empty($reg['retificacao_de_id'])): ?>
                        <a href="<?= Url::pagina('historico_prontuario.php') ?>?empresa_id=<?php echo $empresaId; ?>&prontuario_id=<?php echo $reg['id']; ?>"
                            style="color:#7f8c8d; text-decoration:none; font-size:13px; display:inline-block; margin-top:8px;">🕒
                            Ver histórico de versões</a>
                    <?php endif; ?>
                    <?php if (!$readOnly): ?>
                        <a href="<?= Url::pagina('retificar_prontuario.php') ?>?prontuario_id=<?php echo $reg['id']; ?>&empresa_id=<?php echo $empresaId; ?>"
                            class="btn-rectify"
