...
$limiteAtingido = $produtoController->limiteAtingido($empresaId);
$totalProdutos  = $produtoController->contarProdutosPorEmpresa($empresaId);
$nomePlano      = $empresa['plano_nome'] ?? 'Grátis';
$limitePlano    = $empresa['plano_limite_produtos'] ?? null;
$trialVencido   = $empresaController->trialVencido($empresaId);
$cadastroBloqueado = $limiteAtingido || $trialVencido;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!Csrf::validar($_POST["csrf_token"] ?? null)) {
        ...
        $mensagem     = "O período de teste do seu plano venceu. Escolha um plano pago pra continuar cadastrando produtos.";
        $tipoMensagem = "erro";

    } elseif ($limiteAtingido) {

        $mensagem     = "Você atingiu o limite de {$limitePlano} produtos do plano {$nomePlano}. Faça upgrade de plano pra cadastrar mais.";
        $tipoMensagem = "erro";
...
<p class="small text-muted">
    Plano <strong><?= htmlspecialchars($nomePlano) ?></strong> —
    <?= $limitePlano !== null ? "{$totalProdutos} de {$limitePlano} produtos usados" : "produtos ilimitados" ?>
</p>

<?php if ($trialVencido): ?>

<div class="mensagem erro">
    ⏰ O período de teste do seu plano venceu.
    <a href="<?= Url::pagina('planos.php') ?>?empresa_id=<?= $empresaId ?>">Escolha um plano pago</a> pra continuar cadastrando produtos.
</div>

<?php elseif ($limiteAtingido): ?>

<div class="mensagem erro">
    Você atingiu o limite de <strong><?= (int) $limitePlano ?></strong> produtos do plano <strong><?= htmlspecialchars($nomePlano) ?></strong>.
    <a href="<?= Url::pagina('planos.php') ?>?empresa_id=<?= $empresaId ?>">Veja os planos com catálogo maior</a> pra continuar cadastrando.
</div>
