...
// Se veio de uma consulta já agendada (fluxo novo, tutor pediu o agendamento),
// reaproveita ela em vez de criar uma consulta duplicada do zero.
$consultaExistenteId = (int) ($_POST['consulta_id'] ?? 0);

if ($consultaExistenteId > 0) {
    $stmtConsultaCheck = $pdo->prepare("SELECT id, empresa_id, pet_id FROM consultas WHERE id = ? LIMIT 1");
    $stmtConsultaCheck->execute([$consultaExistenteId]);
    $consultaCheck = $stmtConsultaCheck->fetch(PDO::FETCH_ASSOC);

    if (!$consultaCheck || (int) $consultaCheck['empresa_id'] !== $empresaId) {
        die("Erro: consulta inválida para esta empresa.");
    }

    // Se a consulta já tinha um pet definido, trava nele (ignora o POST).
    // Se não tinha (tutor não escolheu pet ao agendar), mantém o que o
    // veterinário selecionou manualmente no formulário.
    if (!empty($consultaCheck['pet_id'])) {
        $petId = (int) $consultaCheck['pet_id'];
    }
}
...
try { 
    ...
    if ($consultaExistenteId > 0) {
        // 2. Reaproveita a consulta existente, só garantindo que ela feche como Concluída
        $stmtAtualizaConsulta = $pdo->prepare("
            UPDATE consultas
            SET status = 'Concluída', motivo = ?, observacoes = ?
            WHERE id = ?
        ");
        $stmtAtualizaConsulta->execute([$motivo, $diagnostico, $consultaExistenteId]);
        $novoIdConsulta = $consultaExistenteId;
    } else {
