                ...
                <?php if (count($petsDoUsuario) > 0): ?>
                    <?php foreach ($petsDoUsuario as $p): ?>
                        <div style="display:flex; align-items:center; gap:12px; padding:8px 0; border-bottom:1px solid #f1f1f1;">
                            <?php if (Foto::existe($p['foto'], 'pets')): ?>
                                <img src="<?= htmlspecialchars(Foto::url($p['foto'], 'pets')) ?>" width="40" height="40" style="border-radius:50%; object-fit:cover;">
                            <?php else: ?>
                ...
                <?php if (count($empresasDoUsuario) > 0): ?>
                    <?php foreach ($empresasDoUsuario as $emp): ?>
                        <div style="display:flex; justify-content:space-between; align-items:center; padding:8px 0; border-bottom:1px solid #f1f1f1;">
                            ...
                            <a href="<?= Url::pagina('suporte_admin.php') ?>#empresa-<?= (int) $emp['id']; ?>" class="btn-acao" style="background:#e67e22; color:white;" title="Acessar via personificação (com justificativa, fica registrado na auditoria)">🛠️ Acessar via Suporte</a>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
